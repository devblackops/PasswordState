function Get-PasswordStatePasswordHistory {
    ...
    .EXAMPLE
        PS C:\> $history = Get-PasswordStatePasswordHistory -ApiKey $key -PasswordId 1234 -Endpoint 'https://passwordstate.local'

        Retrieve history for password ID 1234.
    .EXAMPLE
        PS C:\> $history = Get-PasswordStatePasswordHistory -ApiKey $key -PasswordId $id -Endpoint 'https://passwordstate.local' -format xml

        Retrieve history for password ID $id in XML format.
    .EXAMPLE
        PS C:\> Get-PasswordStatePasswordHistory -ApiKey $key -PasswordId 1234 -Endpoint 'https://passwordstate.local' | fl
