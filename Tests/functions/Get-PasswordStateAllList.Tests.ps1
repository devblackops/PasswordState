...
InModuleScope 'PasswordState' {
    Describe "Function Get-PasswordStateAllList" {
        BeforeAll {
            $ParameterTests = @(
            ...
            Mock -CommandName 'Invoke-RestMethod' -MockWith {
                $global:TestJson['PasswordStateAllListResponse'] | ConvertFrom-Json
            } -ParameterFilter { $uri -and $uri -match 'passwordlists' -and $ContentType -match 'json'}

            #Mock for invoke-restmethod
            Mock -CommandName 'Invoke-RestMethod' -MockWith {
                [xml]($global:Testxml['PasswordStateAllListResponse'])
            } -ParameterFilter { $uri -and $uri -match 'passwordlists' -and $ContentType -match 'xml'}
        }
        Context 'Analyzing Parameters' {
            It 'Should ensure parameter <param> is declared'-TestCases $ParameterTests {
                param($param)
                ((Get-Command -Name 'Get-PasswordStateAllList').Parameters[$param]) | should -Not -Be $null
            }
            It "Should ensure that the mandatory property for Parameter <param> is set to <mandatory>" -TestCases $ParameterTests {
                ...
                "$((((Get-Command -Name 'Get-PasswordStateAllList').Parameters[$param].Attributes | ? { $_.GetType().fullname -match 'ParameterAttribute'}) | select-object -first 1).Mandatory)" | should -be $mandatory
            }
            It "Should ensure that the format value <value> is accepted" -TestCases $AllParameterFormatTests {
                param($value,$works)
                $value -in ((get-command 'Get-PasswordStateAllList').Parameters['Format'].Attributes | Where-Object { $_.gettype().fullname -eq 'System.Management.Automation.ValidateSetAttribute' }).ValidValues | should be $works
            }
        }
        Context 'Unit Testing function' {
            It "Should return an object with an property 'totalpasswords' for <value> Format" -TestCases $ValidParameterFormatTests {
                param($value, $works)
                (Get-PasswordStateAllList -SystemApiKey $APIKey['GoodKey'] -format $value).totalpasswords | should -not -be $null
            }
        }
    }
}