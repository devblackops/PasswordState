    ...
    Describe "Function Get-PasswordStateListPassword" {
        ...
        Context 'Analyzing Parameters' {
            ...
            It "Should ensure that the format value <value> is accepted" -TestCases $AllParameterFormatTests {
                param($value,$works)
                $value -in ((get-command 'Get-PasswordStateListPassword').Parameters['Format'].Attributes | Where-Object { $_.gettype().fullname -eq 'System.Management.Automation.ValidateSetAttribute' }).ValidValues | should be $works
            }
        }
        Context 'Unit Tests' {
            It "Should return an object with an property 'totalpasswords' for <value> Format" -TestCases $ValidParameterFormatTests {
                param($value, $works)
                (Get-PasswordStateListPassword -ApiKey $APIKey['GoodKey'] -PasswordListId 1 -format $value).totalpasswords | should -not -be $null
            }
        }
    }
}